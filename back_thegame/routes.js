...
router
  .post("/account", function (req, res) {
    ...
    Player.register(login, password, email)
      .then(function () {
        res.status(201).end();
      })
      .catch(function () {
  ...
  .put("/account", Player.isAuthenticated, function (req, res) {
    ...
    Player.editPlayer(
      ...
      .then((result) =>
        res.status(200).json({ login: result.login, mail: result.email })
      )
      .catch((err) => {
        console.log(err);
  ...
  .delete("/account", Player.isAuthenticated, function (req, res) {
    ...
    Player.deletePlayer(playerId)
      .then(() =>
        res
          .clearCookie(Constants.JWT_COOKIE)
          .status(200)
...
router
  .put("/authentication", function (req, res) {
    const { login, password } = req.body;
    Player.authenticate(login, password)
      .then(function (info) {
        let body = {
          id: info.playerId.toString(),
          login: info.playerLogin,
          mail: info.playerMail,
        };
        //res.cookie(Constants.JWT_COOKIE, token, { httpOnly: true /*, secure: true */ }).sendStatus(204);
        res
          .cookie(Constants.JWT_COOKIE, info.token, {
  ...
  .get("/authentication", Player.isAuthenticated, function (req, res) {
    ...
    Player.getPlayerInfo(playerId)
      .then(function (info) {
        res
          .status(200)
          .json({ id: info._id, login: info.login, mail: info.email });
      })
      .catch(function () {
        res.status(403).send("Pseudo ou mot de passe incorrect");
      });
...
router.post("/game", Player.isAuthenticated, function (req, res) {
  Game.createGame(req.body.name, req.body.public, req.body.nbPile)
    .then((game) => res.status(201).json({ id: game._id }))
    .catch((err) => res.sendStatus(500));
});

router
  .put("/game/:id/player", Player.isAuthenticated, function (req, res) {
    const {
      params: { id },
      jwt: { playerId },
    } = req;
  ...
  .delete("/game/:id/player", Player.isAuthenticated, function (req, res) {
    const {
      params: { id },
      jwt: { playerId },
    } = req;
...
router
  .get("/game/:id/card", Player.isAuthenticated, function (req, res) {
    const {
      query: { cardValue },
      params: { id },
      jwt: { playerId },
  ...
  .put("/game/:id/card", Player.isAuthenticated, function (req, res) {
    const {
      body: { cardValue, pileId },
      params: { id },
      jwt: { playerId },
    } = req;
...
router.get("/games/playable", Player.isAuthenticated, function (req, res) {
  const {
    jwt: { playerId },
  } = req;
...
router.delete("/games/deleteAll", Player.isAuthenticated, function (req, res) {
  Game.deleteAllGames();
  res.status(200).json("Delete all games : OK");
});

router.put("/game/:id/message", Player.isAuthenticated, function (req, res) {
  const {
    body: { message },
    params: { id },
    jwt: { playerId },
  } = req;
  Game.sendMessage(id, playerId, message)
    .then(function (result) {
      res.sendStatus(200);
    })
    .catch(function (err) {
